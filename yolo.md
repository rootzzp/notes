...
- [YOLO\_X](#yolo_x)
  ...
  - [Neck](#neck-5)
  - [Head](#head-7)
  - [MMdetection 对应解释](#mmdetection-对应解释-2)
    - [head](#head-8)
- [YOLO\_V6](#yolo_v6)
  - [整体结构：](#整体结构)
  - [Backbone](#backbone-7)
...
class YOLOXHead(BaseDenseHead):
    ...
    def loss_by_feat(
        ...
        flatten_cls_preds = torch.cat(flatten_cls_preds, dim=1) # shape:[batch_size,8400,n_class]
        flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1) # shape:[batch_size,8400,4]
        flatten_objectness = torch.cat(flatten_objectness, dim=1) # shape:[batch_size,8400]
        flatten_priors = torch.cat(mlvl_priors) # shape:[8400,4] (coord_x, coord_y, stride_w, stride_h)
        flatten_bboxes = self._bbox_decode(flatten_priors, flatten_bbox_preds)
