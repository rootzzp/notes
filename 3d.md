...
def forward(self,
        ...
        """
        query [n_query,1,256]
        reference_points [bs,900,3]
        reg_branch: None
        kwargs: [key, # None
                    value, # list:[[1,6,256,60,100],[1,6,256,30,50],[1,6,256,15,25],[1,6,256,8,13]]
                    query_pos, # [n_query,1,256]
                    img_metas]
        """
        output = query
        intermediate = []
        ...
        for lid, layer in enumerate(self.layers):  # iterative refinement
            ...
            output = layer( # 调用下面的forward函数
                output, # [n_query,1,256]
                *args,
                reference_points=reference_points_input, # [bs,900,3]
                **kwargs) # [n_query,bs,256]
            intermediate.append(output)
            intermediate_reference_points.append(reference_points)
...
def forward(self,
                ...
                **kwargs):
        """
        query [n_query,bs,256]
        key   None
        value list:[[bs,6,256,60,100],[bs,6,256,30,50],[bs,6,256,15,25],[bs,6,256,8,13]]
        residual None
        query_pos [n_query,bs,256]
        reference_points [bs,900,3]
        """
        if key is None:
            key = query
        ...
        if query_pos is not None:
            query = query + query_pos # [n_query,bs,256]

        query = query.permute(1, 0, 2) # [bs,n_query,256]

        bs, num_query, _ = query.size()

        # self.attention_weights:Linear(256,24)
        # self.num_cams: 6 self.num_points: 1 self.num_levels: 4
        # attention_weights shape: [bs,1,n_query,6,1,4]
        attention_weights = self.attention_weights(query).view(
            bs, 1, num_query, self.num_cams, self.num_points, self.num_levels)
        
        # reference_points_3d: [bs,900,3]
        # output: [bs,256,900,6,1,4]
        # mask: [bs, 1, 900, 6, 1, 1]
        reference_points_3d, output, mask = feature_sampling( # 见下面的函数
            value, reference_points, self.pc_range, kwargs['img_metas'])
        output = torch.nan_to_num(output)
        mask = torch.nan_to_num(mask)
        attention_weights = attention_weights.sigmoid() * mask # [bs,1,n_query,6,1,4]
        output = output * attention_weights # [bs,256,n_query,6,1,4]
        output = output.sum(-1).sum(-1).sum(-1) # [bs, 256, 900]
        output = output.permute(2, 0, 1) # [900, bs, 256]

        output = self.output_proj(output) # [900, bs, 256]
        pos_feat = self.position_encoder(
            inverse_sigmoid(reference_points_3d)).permute(1, 0, 2) # [900, bs, 256]
...
def feature_sampling(mlvl_feats,
                     ...
                     no_sampling=False):
    """
    mlvl_feats list:[[bs,6,256,60,100],[bs,6,256,30,50],[bs,6,256,15,25],[bs,6,256,8,13]]
    ref_pt     [bs,900,3],有可学习参数query_pos学习到的
    pc_range   [min_x, min_y, min_z, max_x, max_y, max_z]
    ...
    lidar2img = ref_pt.new_tensor(lidar2img) # [1,6,4,4]，只是让lidar2img与ref_pt的divice和dtype相同
    ref_pt = ref_pt.clone() # [bs,900,3]
    ref_pt_3d = ref_pt.clone() # [bs,900,3]
    ...
    ref_pt = torch.cat((ref_pt, torch.ones_like(ref_pt[..., :1])), -1) # [bs,900,4]（齐次坐标）
    ref_pt = ref_pt.view(B, 1, num_query, 4) # [bs,1,900,4]
    ref_pt = ref_pt.repeat(1, num_cam, 1, 1).unsqueeze(-1) # [bs,6,900,4,1]

    lidar2img = lidar2img.view(B, num_cam, 1, 4, 4)\
                         .repeat(1, 1, num_query, 1, 1) # [bs,6,900,4,4]

    pt_cam = torch.matmul(lidar2img, ref_pt).squeeze(-1) # [bs,6,900,4]
    ...
    mask = ( # 合理的坐标点mask,shape: [bs,6,900,1]
        mask & (pt_cam[..., 0:1] > 0.0)
        & (pt_cam[..., 0:1] < 1.0)
        & (pt_cam[..., 1:2] > 0.0)
        & (pt_cam[..., 1:2] < 1.0))
    # mask shape [bs, 1, 900, 6, 1 1]
    mask = mask.view(B, num_cam, 1, num_query, 1, 1).permute(0, 2, 3, 1, 4, 5)
    mask = torch.nan_to_num(mask)

    pt_cam = (pt_cam - 0.5) * 2  # [0,1] to [-1,1] to do grid_sample; shape: [bs,6,900,2]
    sampled_feats = []
    for lvl, feat in enumerate(mlvl_feats):
        B, N, C, H, W = feat.size()
        feat = feat.view(B * N, C, H, W) # [bs*6,256,h,w]
        pt_cam_lvl = pt_cam.view(B * N, num_query, 1, 2) # [bs*6,900,1,2]
        sampled_feat = F.grid_sample(feat, pt_cam_lvl) # [bs*6,256,900,1]
        sampled_feat = sampled_feat.view(B, N, C, num_query, 1)
        sampled_feat = sampled_feat.permute(0, 2, 3, 1, 4) # [bs,256,900,6,1]
        sampled_feats.append(sampled_feat)

    sampled_feats = torch.stack(sampled_feats, -1) # [bs,256,900,6,4]
    sampled_feats = \
        sampled_feats.view(B, C, num_query, num_cam, 1, len(mlvl_feats)) #[bs,256,900,6,1,4]
